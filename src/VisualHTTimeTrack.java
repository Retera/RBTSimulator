...
public class VisualHTTimeTrack extends JPanel {

	VisualHTModel bstModel;
	
	JList inputs;
	
	VisualHTStateListModel inputsModel;
	
	boolean listening = true;
	
	public VisualHTTimeTrack(VisualHTModel bstModel) {
		setPreferredSize(new Dimension(1200,200));
		this.bstModel = bstModel;
		
		inputs = new JList(inputsModel = new VisualHTStateListModel(bstModel));
		inputs.setLayoutOrientation(JList.HORIZONTAL_WRAP);
		inputs.setCellRenderer(new VisualHTStateRenderer(bstModel));
		...
		inputs.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if( listening && index >= 0 ) {
					VisualHTTimeTrack.this.bstModel.seek(index);
				}
			}
		});
